...
//seq RC.2 1
/***
 * This class is a copy of XORCoderPrptocol, to carry the regenerating code
 * matrix. created at 2014-4-12. modified at 2014-4-23
 * 
 * @author ds
 */
public class RegeneratingCodeMatrix extends CodingMatrix {
	...
	public RegeneratingCodeMatrix() {
		...
		byte[][] messageMatrix = { { 1, 2, 3, 7 }, { 2, 4, 5, 8 },
				{ 3, 5, 6, 9 }, { 7, 8, 9, 0 } };
			...
			for (byte column = 0; column < 4; column++) {
				for (byte index = 0; index < 4; index++) {
					if (messageMatrix[index][column] - 1 >= 0) {
						matrix[messageMatrix[index][column] - 1][column + row
								* 4] = vandermondeMatrix[row][index];
					}
	...
	@Override
	public int decoder(short[][] VdcMatrixInShort, byte[][] rawBuf,
			int[] buflen, int offset, byte[] resultBuf) {
...
//		short[][] PdcInverse = RSCoderProtocol.getRSP()
//				.InitialInvertedCauchyMatrix(Pdc);
		short[][] PdcInverse = getInvertedMatrix(Pdc);
		...
		for (int index = 0; index < indexCount; index++) {
			for (int row = 0; row < k_nodes; row++) {
				...
				for (int column = 0; column < d; column++) {
					...
					box[index][row][column] = tmp >= 0 ? (short) tmp
							: (short) (tmp + 256);
				}
			}
		}
	...
	private short[][] matrixMinus(short[][] m1, short[][] m2) {
		if (m1 == null || m2 == null || m1[0].length < m2[0].length
				|| m1.length < m2.length) {
			return null;
		}
	...
	public int getStoreFileNodesNum() {
		...
		for (byte row = 0; row < 6; row++) {
			for (byte column = 0; column < 4; column++) {
				for (byte index = 0; index < 4; index++) {
					if (messageMatrix[index][column] - 1 >= 0) {
						matrix[messageMatrix[index][column] - 1][column * 6
								+ row] = vandermondeMatrix[row][index];
					}
				}
			}
		}
	...
	public byte[][] getVandermondeMatrix() {
		byte[][] vandermondeMatrix = { { 1, 1, 1, 1 }, { 1, 2, 4, 1 },
				{ 1, 3, 2, 6 }, { 1, 4, 2, 1 }, { 1, 5, 4, 6 }, { 1, 6, 1, 6 } };
