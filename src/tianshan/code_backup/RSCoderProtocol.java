...
//	}
	...
	public short[][] initialCauchyMatrix(int k, int n) {
		short[][] G = new short[k][n];
		short[][] E = new short[k][];
	...
	private void swap(int j, byte[][] g, byte[][] E) {
		byte max = g[j][j];
		int i = -1;
		for (int k = j + 1; k < g.length; k++) {
			if (g[k][j] > max) {
	...
	public byte[][] initialInvertedCauchyMatrix(byte[][] g) {
		byte[][] E;
		E = new byte[g.length][g.length];
		...
		for (int i = 0; i < g.length; i++) {
			...
			if (k > 1) {
				for (int j = 0; j < g.length; j++) {
					g[i][j] = (byte) div[g[i][j]][k];
					E[i][j] = (byte) div[E[i][j]][k];
				}
			}
			for (int j = 0; j < g.length; j++) {
				if ((j == i) || (g[j][i] == 0))
				...
				k = 0xff & g[j][i];
				for (int t = 0; t < g.length; t++) {
					g[j][t] = (byte) div[g[j][t]][k];
					g[j][t] ^= g[i][t];
					E[j][t] = (byte) div[E[j][t]][k];
					E[j][t] ^= E[i][t];
				}
			}
		}
	...
	public int decoder(short[][] g, byte[][] Buf, int[] buflen, int offset,
		...
		g = initialInvertedCauchyMatrix(g);
